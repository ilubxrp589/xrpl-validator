//! Credential byte-exact vectors (bundle-driven).
//!
//! Same harness as deposit_preauth_vector.rs: hydrate the bundle's pre-state,
//! apply the one transaction natively, compare every target object byte for
//! byte against mainnet's post-state.
...
// rsiPEXNs, with Expiration): the Credential object carries the pages of
// both owner directories it was inserted into (`sfIssuerNode`, and
// `sfSubjectNode` when the subject is not the issuer) — two ten-byte UInt64
// Finding 187 — #106796939 B4CE24D232CE (rDcohNUmBA sets a DID Data hash):
// `sfFlags` is soeREQUIRED on the DID ledger format, so the created object
// carries `Flags: 0` right after its type. Ours omitted it: 107 bytes against
// mainnet's 112.
#[test]
