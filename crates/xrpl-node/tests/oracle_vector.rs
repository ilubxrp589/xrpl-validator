//! Oracle byte-exact vectors (bundle-driven).
//!
...
// Finding 293 — #107052956 B1EB9B20D12E (rEVSCgct creates price oracle
// document 1, one QUSD/USD pair): `sfFlags` is soeREQUIRED on the Oracle
// ledger format, so the created object carries `Flags: 0` right after its
// type. The create path built the object without it: 172 bytes against
// mainnet's 177, the diff starting at byte 3. Same shape as finding 187 (DID).
#[test]
