//! Byte-exact vector drills for SetRegularKey (2026-09-01).
//!
//! The free form: a master-signed SetRegularKey on an account whose
//! lsfPasswordSpent is clear has a base fee of ZERO, and applying it SPENDS
//! the password — `if (!minimumFee(app, ctx_.baseFee, fees, flags))
//! sle->setFlag(lsfPasswordSpent)` (SetRegularKey.cpp:71) keys on the
//! calculated base fee, not on the fee the sender chose to pay.
...
/// F60's regression guard (#106696325 5B7E148E, the first live-shadow receipt
/// after the F45-F58 deploy): a fresh account's master-signed SetRegularKey
/// paying 12 drops. Mainnet's AccountRoot lands with Flags 65536
/// (lsfPasswordSpent) and the RegularKey; ours kept Flags 0 — the one
/// byte-diff node of that ledger, PRE-OK, ours-len == ffi-len.
#[test]
fn master_signed_regular_key_spends_the_password_is_byte_exact() {
    run_bundle(include_str!("vectors/regkey_password_spent_106696325.json"));
}
